...
public class SavingsPanel extends JPanel {
    ...
    private void handleCreateGoal() {
        ...
        try {
            ...
            // Automatically trigger System Notification when Goal is created
    ...
    private void handleDepositToGoal(JTable table) {
        ...
        try {
            ...
            // Withdraw from main wallet balance to contribute to savings goal
            mainFrame.getWalletService().withdrawMoney(wallet, amount, "Contribution to Savings Goal: " + goal.getGoalName());

            // Add savings to goal
            mainFrame.getSavingsGoalService().addSavings(goal, amount);

            boolean isCompletedNow = mainFrame.getSavingsGoalService().isGoalCompleted(goal);

            if (!wasCompletedBefore && isCompletedNow) {
                // Award points only upon achieving/completing the goal
                mainFrame.getNotificationService().createNotification(
                        mainFrame.getCurrentUser().getUserId(),
                        "Savings Goal Achieved: " + goal.getGoalName() + " (Target: ₹" + goal.getTargetAmount() + ")",
                        NotificationType.SAVINGS_ALERT
                );

                mainFrame.getRewardService().addReward(
                        mainFrame.getCurrentUser().getUserId(),
    ...
    private void handleDeleteGoal(JTable table) {
        ...
        int goalId = (int) table.getValueAt(selectedRow, 0);
        try {
            mainFrame.getSavingsGoalRepository().delete(goalId);
