...
public class TransactionPanel extends JPanel {
    ...
    public void refreshData() {
        ...
        List<Transaction> transactions = mainFrame.getTransactionRepository().findByWalletId(wallet.getWalletId());
        tableModel.setRowCount(0);
        for (Transaction t : transactions) {
            tableModel.addRow(new Object[]{t.getTransactionId(), t.getTransactionType(), "₹" + t.getAmount().toPlainString(), t.getTransactionDate().toString(), t.getDescription()});
        }
    }
    ...
    private void handleDeposit() {
        ...
        try {
            ...
            mainFrame.getWalletService().depositMoney(wallet, amt, desc);
    ...
    private void handleWithdraw() {
        ...
        try {
            ...
            mainFrame.getWalletService().withdrawMoney(wallet, amt, desc);

            // Automatically log System Notification
            mainFrame.getNotificationService().createNotification(
                    mainFrame.getCurrentUser().getUserId(),
                    "Withdrawal Completed: -₹" + amt + " (" + desc + ")",
                    NotificationType.TRANSACTION_ALERT
            );
